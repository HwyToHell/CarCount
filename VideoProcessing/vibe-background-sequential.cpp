...
vibeModel_Sequential_t *libvibeModel_Sequential_New()
{
  ...
  model->numberOfSamples         = 40; // std: 20
...
int32_t libvibeModel_Sequential_AllocInit_8u_C1R(
  ...
  for (int index = width * height - 1; index >= 0; --index) {
    ...
    for (size_t x = 0; x < model->numberOfSamples - NUMBER_OF_HISTORY_IMAGES; ++x) {
  ...
  uint8_t *segmentation_map)
{
  ...
  for (int index = width * height - 1; index >= 0; --index) {
    if (abs_uint(image_data[index] - historyImage[index]) > (int)matchingThreshold)
...
int32_t libvibeModel_Sequential_Update_8u_C1R(
  ...
  for (y = 1; y < height - 1; ++y) {
    ...
    while (indX < width - 1) {
      ...
      if (updating_mask[index] == COLOR_BACKGRND) {
