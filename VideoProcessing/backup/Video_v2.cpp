...
int _main(int argc, char* argv[])
{
	...
	cfg.changeParam("framesize_x", to_string(cap.get(CV_CAP_PROP_FRAME_WIDTH));
	cfg.changeParam("framesize_y", to_string(cap.get(CV_CAP_PROP_FRAME_HEIGHT));

	// define region of interest --> TODO: select graphical -or- detect by optical flow
	cv::Rect roi(80, 80, 180, 80);
	cfg.changeParam("roi_x", to_string(roi.x));
	cfg.changeParam("roi_y", to_string(roi.y));
	cfg.changeParam("roi_width", to_string(roi.width));
	cfg.changeParam("roi_height", to_string(roi.height));
